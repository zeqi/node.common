...
let log = debug('node.common:helpers:DataHelper:log');
let err = debug('node.common:helpers:DataHelper:error');
...
class DataHelper {
  ...
  static get(url, condition, callback) {
    ...
    return self.httpClient.get(path).then(data => {
      ...
      if (Object.keys(result).length < 1 && (typeof data.text == 'string')) {
